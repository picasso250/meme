...
{
    if (DEBUG) {
        $is_ajax = isset($GLOBALS['is_ajax']) && $GLOBALS['is_ajax'];
        if (!$is_ajax) {
            echo '<p><pre>';
        }
...
function formatSimpleMarkdown($text)
{
    ...
    foreach ($lines as $line) {
        $line = boldFormat($line);
        $a[] = "$line";
    }
    return implode("\n", $a);
}
function boldFormat($line)
{
    $is_pre = strlen($line) > 1 && $line[0] == '>';
    if ($is_pre) {
    ...
    for ($i = 0; $i < $n; $i++) {
        ...
        if ($c == '\\') {
            ...
            if (mb_substr($line, $i, 1)) {
                $cs[] = mb_substr($line, $i, 1);
            }
        } else if ($c == '*') {
            $cs[] = $state == 0 ? "<strong>" : "</strong>";
            $state = 1 - $state;
        } else {
